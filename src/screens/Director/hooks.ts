...
/**
 * Hook to fetch director data from a remote server using TanStack Query as a
 * data fetching tool.
 *
...
const useFetchDirector = (slug: string): FetchDirectorState => {
  const fetchDirector = async (): Promise<Director> => {
    return fetcher<Director>(`${BASE_URL}${DIRECTOR}/${slug}`);
  };

  const {data, error, isLoading, refetch} = useQuery({
    queryKey: ['director', slug],
    queryFn: fetchDirector,
  });
