...
Data is fetched using a `fetch` client and stored in a component's `state` property. Data that needs to be accessed across multiple screens is passed via route parameters, component `props` or observable `MobX State Tree` stores

Directors data is stored in the `DirectorsStore` and data and actions concerning `DirectorsFilter` in it's appropriate `DirectorsFilterStore`
