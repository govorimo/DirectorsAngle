...
Data is fetched using a `fetch` client and stored in a component's `state` property. Data that needs to be accessed across multiple screens is passed via route parameters, component `props` or passed to the
MobX store instance - `DirectorsStore`, or `DirectorsFitlerStore`.

The directors data is stored in `DirectorsStore`, which holds the main list of directors as well as all actions concerning the `DirectorsAngle` screen. For filtering functionality on the `DirectorsFilter` screen, `DirectorsFilterStore` manages the filtered data.
