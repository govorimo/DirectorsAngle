...
Data is fetched using a `fetch` client and stored in a component's `state` property. Data that needs to be accessed across multiple screens is passed via route parameters, component props, or `Recoil` atoms.

Directors data is stored in a `Recoil` atom, every piece of state for example on `DirectorsAngle` screen having its own atom as well, as every action does.

`DirectorsState` contains atoms concerning `DirectorsAngle` and `DirectorsFitlerState` atoms concerning `DirectorsFilter` screen
